let photoView = (function () {
			...
			.then(
				response => {
					...
					photoModel.likePhotoPostPromise(post.id)
						.then(response => {},
							error => {throw new Error(`rejected: ${error}`);}
						);
					photoModel.likePhotoPostPromise(post.id)
						.then(response => {
					...
					document.getElementById('divInnerViewPhoto').innerHTML += html;
				},
				error => {throw new Error(`rejected: ${error}`);}
			);
		window.scrollTo(0, 0);
	...
	let addPhotoPost = () => {
		...
		document.getElementById('hidden-content').innerHTML = html;
		if (!user) {
			document.getElementById('divInnerAddPhoto').innerHTML += 'Only authorized users can add photos';
			return false;
		}
		html = `
		...
		document.getElementById('divInnerAddPhoto').innerHTML += html;
	};
			...
			.then(
				response => {
				   ...
				   <input id="submitEditPost" type="submit" value="save"  style="width:40%;" class="add-btn"/>
					...
					document.getElementById('divInnerEditPhoto').innerHTML += html;
				},
				error => {throw new Error(`rejected: ${error}`);}
			);
		window.scrollTo(0, 0);
	...
	let filtHashtag = (t) => {
		document.getElementById('filt-input-hashtag').value =  t.innerHTML;
		photoView.genPhotoPosts();
	...
	let genLogForm = () => {
		...
		document.getElementById('divOuterLogin').remove();
